...
export default function DashboardPage() {
  ...
  const fetchLogs = async () => {
    try {
      const { data, error } = await supabase
        .from("riwayat")
        .select("*")
        .is("deleted_at", null) // HANYA AMBIL YANG TIDAK DIHAPUS
  ...
  const fetchTrash = async () => {
    try {
      const { data, error } = await supabase
        .from("riwayat")
        .select("*")
        .not("deleted_at", "is", null) // HANYA AMBIL YANG DI TEMPAT SAMPAH
        .order("deleted_at", { ascending: false });

      if (error) throw error;

      if (data) {
        // LOGIKA AUTO-DELETE 7 HARI
        ...
        for (const item of data) {
          ...
          if (diffDays >= 7) {
            // Hapus permanen jika sudah 7 hari
  ...
  // Panggil fetch otomatis saat menu diklik
  useEffect(() => {
  ...
  const handleSaveResult = async () => {
    ...
    try {
      ...
      if (error) throw error;
      showToast("✅ Hasil berhasil disimpan ke Database!");
      setCameraMode("input");
      fetchLogs();
    } catch (err: any) {
  ...
  const handleSoftDelete = async (id: number) => {
    const isConfirm = window.confirm(
      "Pindahkan riwayat ini ke Tempat Sampah? Data bisa dikembalikan dalam 7 hari.",
    );
    if (!isConfirm) return;
  ...
  const handleExportExcel = () => {
    ...
    let csv =
      "Waktu,Status,Jenis Makanan,Petugas,Akurasi,Kalori (kkal),Protein (g),Lemak (g),Karbo (g)\n";
    logEntries.forEach((log) => {
      const waktu = new Date(log.created_at)
        .toLocaleString("id-ID")
        .replace(/,/g, " ");
      csv += `${waktu},${log.status},${log.jenis_makanan},${log.petugas_nama},${Math.round(log.confidence * 100)}%,${log.kalori},${log.protein},${log.lemak},${log.karbo}\n`;
    });
      ...
      <div className="dashboard-wrapper">
        ...
        <aside className={`sidebar ${isSidebarOpen ? "open" : ""}`}>
          ...
          <nav className="sidebar-nav">
            ...
            <button
              className={`nav-item ${activePage === "sampah" ? "active" : ""}`}
              onClick={() => {
          ...
          <div className="top-bar">
            <div className="step-indicators">
              <div className="step-item">
                ...
                <span className="step-label">1. Arahkan Kamera</span>
              </div>
              <div className="step-item">
                <div className="step-icon">📸</div>
                <span className="step-label">2. Tekan Ambil Foto</span>
              </div>
              <div className="step-item">
                <div className="step-icon">📊</div>
                <span className="step-label">3. Lihat Hasil Gizi</span>
              </div>
            </div>
              ...
              <div className="dash-view active">
                <div style={{ textAlign: "center", padding: "3rem 1rem" }}>
                  <h2 style={{ fontSize: "1.6rem", marginBottom: "0.5rem" }}>
                    Selamat Datang di MBG Smart System
                  </h2>
                  <p
                    style={{
                  ...
                  >
                    Gunakan menu <strong>Kamera Langsung</strong> untuk memulai
                    analisis kualitas dan gizi makanan. Sistem akan mendeteksi
                    kesegaran makanan dan menampilkan informasi nutrisi secara
                    real-time.
                  </p>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        Seluruh kalkulasi gizi pada MBG Smart System menggunakan
                        standar perhitungan mutlak berdasarkan{" "}
                        <strong>Tabel Komposisi Pangan Indonesia (TKPI)</strong>{" "}
                        yang diterbitkan resmi oleh{" "}
                        <strong>
                          Kementerian Kesehatan Republik Indonesia
                        </strong>
                        . Data dihitung per 100 gram BDD (Berat Dapat Dimakan).
                      </p>
                    </div>
                    ...
                    >
                      Daftar Kandungan Gizi (Menu Basis Data AI)
                    </h3>
                    <div
                      style={{
                        overflowX: "auto",
                    ...
                    >
                      *Nilai di atas adalah takaran per 100 gram bahan sesuai
                      panduan TKPI Kemenkes RI.
                    </p>
                  </div>
                </div>
              </div>
                      ...
                      <>
                        <div className="quality-card">
                          <div className="quality-header">
                            <span
                              className={`badge ${detectionResult.status === "SEGAR" ? "badge-fresh" : "badge-spoiled"}`}
                            >
                              {detectionResult.status}
                            </span>
                            <span className="quality-indicator">
                              {detectionResult.status === "SEGAR"
                                ? "🟢 Indikator Hijau"
                                : "🔴 Indikator Merah"}
                            </span>
                          </div>
                          ...
                          <div className="nutrition-grid">
                            <div className="nutrition-item">
                              <div className="nut-value">
                                {detectionResult.kalori}
                              </div>
                              <div className="nut-label">Kalori (kkal)</div>
                            </div>
                            <div className="nutrition-item">
                              <div className="nut-value">
                                {detectionResult.protein}
                              </div>
                              <div className="nut-label">Protein (g)</div>
                            </div>
                            <div className="nutrition-item">
                              <div className="nut-value">
                                {detectionResult.lemak}
                              </div>
                              <div className="nut-label">Lemak (g)</div>
                            </div>
                            <div className="nutrition-item">
                              <div className="nut-value">
                                {detectionResult.karbo}
                              </div>
                              <div className="nut-label">Karbo (g)</div>
                            </div>
                          </div>
                        </div>
                        ...
                        <tbody>
                          {logEntries.map((log, i) => (
                            <tr key={i}>
                              ...
                              <td>
                                <span
                                  className={`badge ${log.status === "SEGAR" ? "badge-fresh" : "badge-spoiled"}`}
                                >
                                  {log.status}
                                </span>
                              </td>
              ...
              <div className="dash-view active">
                <div className="log-section">
                  <div style={{ marginBottom: "1.5rem" }}>
                    <h2 style={{ margin: 0, color: "#cb4335" }}>
                      🗑️ Tempat Sampah
                    </h2>
                    <p
                      style={{
                    ...
                    >
                      Item akan dihapus permanen secara otomatis setelah 7 hari.
                    </p>
                  </div>
                            ...
                            return (
                              <tr key={i}>
                                ...
                                >
                                  ...
                                  <button
                                    onClick={() => handleHardDelete(log.id)}
                                    style={{
                                      background: "#e74c3c",
                                      color: "white",
                                      border: "none",
              ...
              <div className="dash-view active">
                <div className="profil-section">
                  <div className="profil-card">
                    <div className="profil-avatar-big">
                      {currentUser.nama_lengkap.charAt(0).toUpperCase()}
                    </div>
                    <div className="profil-row">
                      <span className="row-label">Nama Lengkap</span>
                    ...
                    <div className="profil-row">
                      ...
                      <span className="row-value">{currentUser.username}</span>
                    </div>
                    <div className="profil-row">
                      <span className="row-label">Role</span>
                      <span className="row-value">{currentUser.role}</span>
                    </div>
                    <div className="profil-row">
                      <span className="row-label">Lokasi</span>
                      <span className="row-value">{currentUser.lokasi}</span>
                    </div>
                    <div className="profil-row">
                      <span className="row-label">No. HP</span>
                      <span className="row-value">{currentUser.no_hp}</span>
                    </div>
                    <div
          ...
          <div className="modal-overlay">
            <div className="modal-card">
              ...
              <form onSubmit={handleSaveProfil}>
                ...
                <div className="form-field">
                  <label className="form-label">Lokasi Dapur MBG</label>
                  <input
                    type="text"
                ...
                <div className="form-field">
                  <label className="form-label">Nomor Telp / HP</label>
                  <input
                    type="text"
        ...
        {toastMsg && (
          <div className="toast" style={{ display: "block", opacity: 1 }}>
            {toastMsg}
          </div>
        )}
      </div>
